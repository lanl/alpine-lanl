## This script tracks a single bubble foreward and backward
################################################################
...
def extract_target_feature(feature_volume,confidence_th,feature_id):
    ...
    feature_volume.GetPointData().SetActiveScalars('similarity')
...
########################
## for high res data
feature_path = '../out/mfix_simfields_highres/'
feature_fname = feature_path + 'insitu_simfield_' + str(feature_tstep) + '.vti'
print ('initial feature file to load: ' + feature_fname)

data_path = '../out/segmented_features_highres/'
full_cdb_path = '../bubble_all_highres.cdb/'
segmented_feature_volumes = '../out/segmented_volumes_highres/'
...
forward_tracking = []
for ii in range(feature_tstep+10,endT,10):
...
backward_tracking = []
for ii in range(feature_tstep-10,initT,-10):
...
all_tracked_results.append([float(feature_tstep),float(feature_id),avg_dice,avg_vel])
...
for i in range(len(all_tracked_results)):
    #print (all_tracked_results[i][0],all_tracked_results[i][1])
