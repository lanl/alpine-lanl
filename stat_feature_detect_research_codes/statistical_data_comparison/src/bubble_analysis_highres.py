...
## This script processes those vti files and generates a picke file for each time step which contains all the feature attributes
...
def compute_bubble_features(fname,confidence_th,size_threshold, tstep, outpath):
    ...
    data = read_vti(fname)
    data.GetPointData().SetActiveScalars('similarity')
    ...
    for i in range(num_segments):
        if  bubble_volumes[i] > size_threshold:
            ...
            ## compute eigen ratio, calling roundness
            ###########################################
            ...
            roundness = 0.0 ## finally not using roundness
    ...
    #print ('top features to remove: ' + str((top_indices)))       
...
input_path = '../out/mfix_simfields_highres/'
#input_path = '/Users/sdutta/Desktop/tmp_test/'
## Folder that will contain the pickle files, vtm files and the csv file
#outpath = '/Users/sdutta/Desktop/tmp_test/'
outpath = '../out/segmented_features_highres/'
...
for ii in range(init_time,end_time,10):
	print ('Processing time step: ' + str(ii))
	inpfname = input_path + 'insitu_simfield_' + str(ii) + '.vti'
...
out_csv_path = outpath + 'all_bubble_info.csv'
