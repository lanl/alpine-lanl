...
for tstep in range(startT,endT,window):
    ...
    if tstep == startT:
        ...
        for i in range(len(var2)):
            ...
            binid1 = int(((var1[i] - minval1)/(maxval1-minval1))*(numBins-1))
            binid2 = int(((var2[i] - minval2)/(maxval2-minval2))*(numBins-1))

            if dataset == 'mfix' or dataset == 'isabel' or dataset == 'tornado' or dataset == 'cylinder':
                if var2[i] < density_th:
                    I1Arr.InsertNextTuple1(PMI[binid1][binid2])
...
out_fname = outpath + 'fused_pmi.vti'
