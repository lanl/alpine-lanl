...
def compute_pointwise_mutual_information(Array1,Array2,ArrayComb,numSamples,bins):
    ...
    for i in range(bins):
        for j in range(bins):
            ...
            if ArrayComb[i][j]==0:
                pmi_array[i][j]=0
            elif ArrayComb[i][j] > 0.0 and Array1[i] > 0.0  and Array2[j] > 0.0:
...
def compute_pointwise_mutual_information_3d(Array1,Array2,Array3,ArrayComb,numSamples,bins):
    pmi_array = np.zeros_like(ArrayComb)
    ...
    for i in range(bins):
        for j in range(bins):
            for k in range(bins):
                ...
                if ArrayComb[i][j][k]==0:
                    pmi_array[i][j][k]=0
                elif ArrayComb[i][j][k] > 0 and Array1[i] > 0  and Array2[j] > 0 and Array3[i] > 0:
                    prob_x = Array1[i]/float(numSamples)
                    prob_y = Array2[j]/float(numSamples)
                    prob_z = Array3[j]/float(numSamples)
                    joint_prob_xyz = ArrayComb[i][j][k]/float(numSamples)
                    pmi_array[i][j][k] = np.log2(joint_prob_xyz/(prob_x*prob_y*prob_z))
                else:
                    pmi_array[i][j][k]=0
