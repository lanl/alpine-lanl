...
float tnorm_product(float *fire, int len)
{
    ...
    // float weights[5] = {0.6,0.1,0.1,0.15,0.005};
    // for(int i=0;i<len;i++)
    //     fire[i] = weights[i]*fire[i];
...
void compute_feature_attribute_variations(vector<Feature_class> current_feature_list, map<float,Feature_vector> *feature_prop,
                                          ...
                                          vector<float> *distance_blob, vector<float> *distance_cbbox, vector<float> *distance_shape, float maxdist)
{
    ...
    for(int i=0; i<current_feature_list.size(); i++)
    {
        ...
        //PCA on the candidate object
        /////////////////////////////////////////////////////////////////
        Mat compressed;
        vector<PointClass> pca_trans_feature;
        Mat time_line_mat(current_feature_list[i].pca_transformed.size(),3,CV_32FC1);
        for(int j=0; j<current_feature_list[i].pca_transformed.size(); j++)
        {
            time_line_mat.at<float>(j,0) = current_feature_list[i].pca_transformed[j].x;
            time_line_mat.at<float>(j,1) = current_feature_list[i].pca_transformed[j].y;
            time_line_mat.at<float>(j,2) = current_feature_list[i].pca_transformed[j].z;
        }
        ...
        for(int k=0;k<compressed.rows;k++)
        {
            ...
            pca_trans_feature.push_back(pt);
        }

        //PCA on target feature
        /////////////////////////////////////////////////////////////////
        Mat compressed1;
        vector<PointClass> pca_trans_target;
        Mat time_line_mat1(fstar.pca_transformed.size(),3,CV_32FC1);
        for(int j=0; j<fstar.pca_transformed.size(); j++)
        {
            time_line_mat1.at<float>(j,0) = fstar.pca_transformed[j].x;
            time_line_mat1.at<float>(j,1) = fstar.pca_transformed[j].y;
            time_line_mat1.at<float>(j,2) = fstar.pca_transformed[j].z;
        }
        //Do PCA: compressed matrix will have the required values
        PCA_analysis(time_line_mat1, 3, time_line_mat1, compressed1);
        for(int k=0;k<compressed1.rows;k++)
        {
            PointClass pt;
            pt.x=round(compressed1.at<float>(k,0));
            pt.y=round(compressed1.at<float>(k,1));
            pt.z=round(compressed1.at<float>(k,2));
            pca_trans_target.push_back(pt);
        }

        feature5 = dice_sim(pca_trans_target,pca_trans_feature);
        ...
        if(feature1 < neighborhood)
        {
            ...
            fvector.feature_vec.push_back(feature5);
    ...
    for(int i=0; i<current_feature_list.size(); i++)
    {
        ...
        for(int j=i+1; j<current_feature_list.size(); j++)
        {
            ...
            if(distance1 < neighborhood && distance2 < neighborhood) //consider a local neighborhood value depends on dataset
            {
                ...
                /////////////////////////////////////////////////////////////////////////////////////////
                // Write the csv file of PCA output:
                /////////////////////////////////////////////////////////////////////////////////////////
                ...
                for(int k=0;k<compressed_new.rows;k++)
                    comb_file_ptr<<round(compressed_new.at<float>(k,0))<<","<<round(compressed_new.at<float>(k,1))
                                <<","<<round(compressed_new.at<float>(k,2))<<endl;
...
map<int,glm::vec2> create_feature_combination_new(vector<Feature_class> current_feature_list,
                                                  Feature_class fstar, float maxdist, int timestep, int found_match, map<float,Feature_vector> *feature_prop)
{
    map<int,glm::vec2> feature_comb_map;
    stringstream tt;
    tt<<timestep;
    ...
    {
        PointClass pt;
        pt.x=round(compressed_target.at<float>(k,0));
        pt.y=round(compressed_target.at<float>(k,1));
        pt.z=round(compressed_target.at<float>(k,2));
        pca_trans_feature.push_back(pt);
    }
    ...
    for(int i=0; i<current_feature_list.size(); i++)
    {
        ...
        for(int j=i+1; j<current_feature_list.size(); j++)
        {
            ...
            if(distance1 < neighborhood && distance2 < neighborhood) //consider a local neighborhood value depends on dataset
            {
                ...
                glm :: vec3 new_center = get_center(combined_feature);

                //compute attributes of combined feature here

                //vel
                float temp = (new_center.x-fstar.cog[0])*(new_center.x-fstar.cog[0]) + (new_center.y-fstar.cog[1])*(new_center.y-fstar.cog[1]) + (new_center.z-fstar.cog[2])*(new_center.z-fstar.cog[2]);
                feature1 = sqrt(temp)/maxdist;

                //mass
                feature2 = fabs(current_feature_list[i].mass + current_feature_list[j].mass - fstar.mass)/fstar.mass;

                //volume
                feature3 = fabs(current_feature_list[i].numofpts + current_feature_list[j].numofpts - fstar.numofpts)/fstar.numofpts;

                //dist cbbox: same as diff center at this time
                feature3 = feature1;
                ...
                vector<PointClass> pca_combined_obj;
                for(int k=0;k<compressed_new.rows;k++)
                {
                    PointClass pt;
                    pt.x=round(compressed_new.at<float>(k,0));
                    pt.y=round(compressed_new.at<float>(k,1));
                    pt.z=round(compressed_new.at<float>(k,2));
                    pca_combined_obj.push_back(pt);
                }

                feature5 = dice_sim(pca_combined_obj,pca_trans_feature);

                cout<<"combined: "<<feature1<<" "<<feature2<<" "<<feature3<<" "<<feature4<<" "<<feature5<<endl;
