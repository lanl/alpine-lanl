...
// New way of normalizing the array, more efficient, does not produce warning while running in GPU
struct NormalizeArrayNew : public vtkm::worklet::WorkletMapField {
    ...
    public:
        ...
        // constructor
        VTKM_EXEC_CONT
        NormalizeArrayNew(vtkm::Float64 minval, vtkm::Float64 maxval) : min(minval), max(maxval) {}

        template<typename Point,
                 typename OutType>
        VTKM_EXEC void operator()(const Point& data_val,
                                  OutType& out) const
        {
            out = (data_val-min)/(max-min);
        }
}; 
...
struct AssignClusterIDs : public vtkm::worklet::WorkletMapField {
    ...
    public:
        ...
        // constructor
        VTKM_EXEC_CONT
        AssignClusterIDs(vtkm::Id3 dims,vtkm::Id3 blockdims)
        : xdim(dims[0]), ydim(dims[1]), zdim(dims[2]), 
        xblock(blockdims[0]), yblock(blockdims[1]), zblock(blockdims[2]) {}
        ...
        VTKM_EXEC void operator()(const Point& index,
                                  OutType& out) const
        {
            vtkm::Id numblockx = xdim/xblock;
            vtkm::Id numblocky = ydim/yblock;
            vtkm::Id numblockz = zdim/zblock;

            vtkm::Id xval = index%xdim;
            vtkm::Id yval = (index/xdim)%ydim;
            vtkm::Id zval = index/(ydim*xdim);


            vtkm::Id blockx = vtkm::Floor(xval/xblock);
            vtkm::Id blocky = vtkm::Floor(yval/yblock);
            vtkm::Id blockz = vtkm::Floor(zval/zblock);

            out = vtkm::Int32(blockx + numblockx*(blocky + numblocky*blockz));
        }
}; 


struct CopyCentersTo1DArray : public vtkm::worklet::WorkletMapField {
    ...
    public:
        ...
        // constructor
        VTKM_EXEC_CONT
        CopyCentersTo1DArray() = default;

        template<typename FieldPortal,
                 typename OutType>
        VTKM_EXEC void operator()(const FieldPortal& pointLoc,
                                  OutType& out1,
                                  OutType& out2,
                                  OutType& out3) const
        {
            //std::cout<<pointLoc[0]<<std::endl;
            out1 = pointLoc[0];
            out2 = pointLoc[1];
            out3 = pointLoc[2];
        }
