...
int Slic ::  getClosestClusterID(vector<float> list)
{
    int id=0;
    float val=list[0];
...
void Slic :: computeSlic(float*** raw_data1)
{
    ...
    while((epsilon > halt_condition) && (iter < iterLimit))
    {
        ...
        for(int k=0;k<zdim;k++)
            for(int j=0;j<ydim;j++)
                for(int i=0;i<xdim;i++)
                {
                    ...
                    //if(processedElem[i][j][k]==0 || this->cluster_ids[i][j][k] < 0)
                    {
                        ...
                        for(int p=0;p<this->clusterCenters.size();p++)
                        {
                            ...
                            e.z = k;
                            e.var1 = raw_data1[i][j][k]*QUANTIZATION;
                            distVector[p] = e.getDistanceEuclid(e,this->clusterCenters[p],weight,blockXSize);
                        }

                        this->cluster_ids[i][j][k] = getClosestClusterID(distVector);
        ...
        for(int k=0;k<zdim;k++)
            for(int j=0;j<ydim;j++)
                for(int i=0;i<xdim;i++)
                {
                    ...
                    tempClusterCenters[this->cluster_ids[i][j][k]].var1 += raw_data1[i][j][k]*QUANTIZATION;
        ...
        cout<<"Iteration: "<<iter<<" epsilon is: "<<epsilon<<endl;
